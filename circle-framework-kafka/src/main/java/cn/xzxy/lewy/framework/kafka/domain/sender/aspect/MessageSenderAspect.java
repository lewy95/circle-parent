...
public class MessageSenderAspect {
    ...
    @Pointcut("@annotation(cn.xzxy.lewy.framework.kafka.domain.sender.aspect.MessageSender)")
    public void messageSender() {
    }

    @AfterReturning("messageSender()")
            ...
            if (TransactionSynchronizationManager.isSynchronizationActive()) {
                TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronizationAdapter() {
                    public void afterCompletion(int status) {
                        if (0 == status) {
                            MessageSenderAspect.this.sendMessage(messageSenderLogs);
                        } else {
                            MessageSenderAspect.log.debug("线程{}提交事务失败，更新{}条消息状态CANCEL", Thread.currentThread().getName(), messageSenderLogs.size());
                            MessageSenderAspect.this.applicationEventPublisher.publishEvent(SenderEvent.cancel(new SendException("事务提交失败，取消发送消息"), messageSenderLogs));
                        }
